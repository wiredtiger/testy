...
def create_table_cycle(connection, interval_s, table_config):
    while is_running:
        time.sleep(interval_s)
        num_tables = len(tables)
        create_table(connection, num_tables, num_tables + 1, table_config)
...
# Create a collection every minute.
interval_s = 2
thread = pythread.Thread(target=my_test, args=(connection, interval_s, table_config))
threads.append(thread)
is_running = True
thread.start()
time.sleep(20)
